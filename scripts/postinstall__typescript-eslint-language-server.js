...
const tsserverPluginPath = path.resolve(npmNodeModules, "./typescript-eslint-language-service")
...
if (!fs.existsSync(targetLibraryPath)) {
  ...
  const generatedLibraryPath = path.resolve(tsserverPluginPath, "./lib/index.js")
  if (fs.existsSync(generatedLibraryPath)) {
    fs.symlinkSync(generatedLibraryPath, targetLibraryPath)
  } else {
    console.error("Post-install script was not able to generate the file for typescript-eslint-language-service.")
  }
}
